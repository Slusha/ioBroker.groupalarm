		...
		this.on("unload", this.onUnload.bind(this));
					...
					async onReady() {
						...
						const determineType = (value) => {
							...
							if (type === 'function' || type === 'symbol' || type === 'undefined') return 'mixed';
							return type;
						};
						...
						this.interval = setInterval(async () => {
							try {
								const response = await axios.get('https://app.groupalarm.com/api/v1/alarms/user', {
									params: {
										limit: 5
									},
									headers: {
								...
								if (response.data && response.data.alarms && Array.isArray(response.data.alarms)) {
